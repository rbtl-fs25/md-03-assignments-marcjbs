...
3.  How many variables are in the data set?
    -   there are 8 variables for the data set
...
```{r}
sanitation_canada_count <- sanitation_canada %>% 
  count(year == 2000, year == 2020)
```
...
sanitation_canada %>% 
  ggplot(
    mapping= aes(x = residence, y = percent, fill = varname_long)
) + 
  geom_col() + 
  facet_wrap(~year) + 
...
1.  Look at the plot that you created. What do you notice about the order of the bars / order of the legend?
    -   the order of the bars is according to the year chosen, the order of the legends is also according to how it is defined in the data set
2.  What would you want to change?
    -   in my bar graph the numbers are not displayed very well, this is the case if there are parts in the bar with very low occurance.
3.  Why did we remove "safely managed sanitation services" from the data set in Task 3?
    -   because it wouldn't fit into the graph?? I have no idea.

## Task 7

1.  Run the code in the code chunk below.
2.  What do you observe when you look at the code and plot?
    -   the code gives an error in the terminal as some rows are removed as they appear to be outside the scale range.
